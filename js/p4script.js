...
function onChangeQuantity(id, e) {
	...
	if (id === 'quantity') {
		idOfItem = "product-offer-perKg"
	} else if (id === 'quantity2') {
		idOfItem = "product-offer-perKg2"
	}

	const pricePerKilo = parseFloat(document.getElementById(idOfItem).innerHTML);
	const price = newQuantity * pricePerKilo;

	totalPrices[id] = price;

	var sumOfPrices = 0;
	Object.keys(totalPrices).forEach(function(key) {
		sumOfPrices += totalPrices[key];
	})
	...
	document.getElementById('sum-before-taxes').innerHTML = "$" + `${sumOfPrices}`;
...
function cartPrice() {
	...
	document.getElementsByClassName("subtotal").innerText = sum.toFixed(2);
